...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if pgnode_a.prenodes.issubset(self.s_levels[level]):
                a_nodes.add(pgnode_a)
                for s_node in self.s_levels[level]:
                    if s_node in pgnode_a.prenodes:
                        pgnode_a.parents.add(s_node)
                        s_node.children.add(pgnode_a)
    ...
    def add_literal_level(self, level):
        ...
        for pgnode_a in self.a_levels[level - 1]:
            for cur_s_node in pgnode_a.effnodes:
                s_nodes.add(cur_s_node)
                pgnode_a.children.add(cur_s_node)
                cur_s_node.parents.add(pgnode_a)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return not set(node_a1.action.effect_add).isdisjoint(node_a2.action.effect_rem) or \
               not set(node_a1.action.effect_rem).isdisjoint(node_a2.action.effect_add)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return not set(node_a1.action.precond_pos).isdisjoint(node_a2.action.effect_rem) or \
               not set(node_a1.action.precond_neg).isdisjoint(node_a2.action.effect_add) or \
               not set(node_a2.action.precond_pos).isdisjoint(node_a1.action.effect_rem) or \
               not set(node_a2.action.precond_neg).isdisjoint(node_a1.action.effect_add)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for s1 in node_a1.parents:
            for s2 in node_a2.parents:
                if s1.is_mutex(s2) or s2.is_mutex(s1):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a1 in node_s1.parents:
            for a2 in node_s2.parents:
                if not a1.is_mutex(a2) or not a2.is_mutex(a1):
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        for level, states in enumerate(self.s_levels):
            for state in states:
                if state.is_pos and state.symbol in self.problem.goal and state.symbol not in seen:
                    level_sum += level
